...
scenario_name = 'DTMNoiseTestHigh2';
...
%% Section Errors
eDTMs = [55 60 65 70 75 80 85 90 95 100];
mean_err_pos = zeros(1, length(eDTMs));
mean_err_rot = zeros(1, length(eDTMs));
idx = 0;
for eDTM = eDTMs
%%
idx = idx + 1;
...
out_dir = [pwd '/Scenario_' scenario_name '/DTM_Noise_' num2str(eDTM)];
mkdir(out_dir);
...
mean_err_pos(idx) = sum(pos_err) / length(pos_err);
mean_err_rot(idx) = sum(rot_err) / length(rot_err);
%%
end
%%
figure;
semilogy(eDTMs, mean_err_pos);
hold on;
semilogy(eDTMs, mean_err_rot);
title('Mean Error vs DTM Noise');
legend('Position Error', 'Rotation Error', 'Location','southeast');
xlabel('DTM Noise (meters)');
ylabel('meters / radians');
hold off;

out_dir = [pwd '/Scenario_' scenario_name];
saveas(gcf,[out_dir '/error_vs_dtm_noise.fig']);
saveas(gcf,[out_dir '/error_vs_dtm_noise.jpg']);