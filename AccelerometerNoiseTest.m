...
out_dir = [pwd '/Scenario_' scenario_name '/Accelerometer_Noise_' num2str(ev)];
mkdir(out_dir);
...
%%
figure;
semilogy(sqrt(3*evs), mean_err_pos);
hold on;
semilogy(sqrt(3*evs), mean_err_rot);
title('Mean Error vs Accelerometer Noise');
legend('Position Error', 'Rotation Error', 'Location','southeast');
xlabel('Accelerometer Noise (meters)');
ylabel('meters / radians');
hold off;

out_dir = [pwd '/Scenario_' scenario_name];
saveas(gcf,[out_dir '/error_vs_accelerometer_noise.fig']);
saveas(gcf,[out_dir '/error_vs_accelerometer_noise.jpg']);